...
Per tal de fer la divisió més adient utilitzant el predictor més adient i el punt de tall optim el model cercarà entre tots els predictors diferents punts de tall per calcular les funcions de cost i elegirà a cada pas el predictor i el punt de tall que minimitzin aquesta funció ($RSS$ en cas de arbres de regressió i Gini en cas de classificació).

\subsection{Arbres de decisió i sobreajustament}

Arribats a aquest punt si continuéssim dividint els nodes podríem arribar a obtenir nodes fulla completament purs els quals només contindrien una sola observació. Aquest arbre tindria un rendiment perfecte a la mostra d'entrenament però estaria clarament sobre sobre-ajustat a aquesta mostra i no donaria bons resultats per noves observacions. Per tant, cal especificar un criteri de parada per tal que l'arbre no creixi massa profund. Dos bons criteris que s'acostumen a utilitzar junts són els següents:
...
\includegraphics[width=12.5 cm, height = 13 cm]{RandomForest.png}
